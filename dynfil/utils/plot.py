...
def plot_comparison(residuums, filenames=tuple(), x='residuum'):
    ...
    for filename in filenames:
        if filename.endswith('.tex'):
            # Output table
            tex = r'\begin{tabular}{l||S[table-format=+1.2e+2]|S[table-format=+1.2e+2]|S[table-format=+1.2e+2]|S[table-format=+1.2e+2]|S[table-format=+1.2e+2]}' + '\n'
            tex += r'{Filter method} & {Mean %s} & {Std. dev.} & {Min.} & {Max.} & {Median}\\' % x
            tex += '\n' + r'\midrule' + '\n'
            for k, v in reversed(residuums.items()):
                tex += '{k} & {mean:.2e} & {sd:.2e} & {min:.2e} & {max:.2e} & {med:.2e}'.format(
                    k=k,
                    mean=np.mean(v),
                    sd=np.std(v),
                    min=np.min(v),
